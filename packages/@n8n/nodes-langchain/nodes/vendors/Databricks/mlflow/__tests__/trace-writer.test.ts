...
describe('trace-level token usage', () => {
	it('sums every model call into mlflow.trace.tokenUsage', () => {
		const withTwoModelCalls = trace();
		withTwoModelCalls.spans = [
			{
				...withTwoModelCalls.spans[0],
				tokenUsage: { inputTokens: 183, outputTokens: 67, totalTokens: 250 },
			},
			{
				...withTwoModelCalls.spans[0],
				span_id: 'other',
				tokenUsage: { inputTokens: 236, outputTokens: 28, totalTokens: 264 },
			},
		];

		const info = buildTraceInfo(withTwoModelCalls, '42');
	...
	it('omits the metadata when no span reported usage', () => {
