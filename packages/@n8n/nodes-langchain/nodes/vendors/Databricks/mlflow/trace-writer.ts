...
/** Adds up every model call in the trace, in MLflow's token-usage key names. */
function sumTokenUsage(
	spans: MlflowSpan[],
): { input_tokens: number; output_tokens: number; total_tokens: number } | undefined {
	const reported = spans.filter((span) => span.tokenUsage !== undefined);
	if (reported.length === 0) return undefined;

	return reported.reduce(
		(total, span) => ({
			input_tokens: total.input_tokens + (span.tokenUsage?.inputTokens ?? 0),
			output_tokens: total.output_tokens + (span.tokenUsage?.outputTokens ?? 0),
			total_tokens: total.total_tokens + (span.tokenUsage?.totalTokens ?? 0),
		}),
...
export function buildTraceInfo(trace: CollectedTrace, experimentId: string) {
	...
	const usage = sumTokenUsage(trace.spans);
	const traceMetadata: Record<string, string> = {};
	if (usage) {
		traceMetadata[MLFLOW_TRACE_METADATA.TokenUsage] = JSON.stringify(usage);
	}
